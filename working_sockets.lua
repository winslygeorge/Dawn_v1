-- dawn_sockets.lua (Enhanced with Phoenix-style Presence + Lifecycle + Timeout + Modular Events + Ack + Binary + Dynamic Rooms)
...
function DawnSockets:syncPrivateChat(user_id, ws)
    ...
    self.state_management:mark_socket_active(ws_id, user_id) -- Mark the socket as active for the user_id
end
...
function DawnSockets:broadcast_to_room(topic, message_table)
    ...
    for ws_id, _ in pairs(room) do -- Iterate over the keys (ws_ids)
        if not message_table.receiver then
            message_table.receiver = self:getSyncPrivateUserID(ws_id) -- Set the receiver to the user_id
        end
...
function DawnSockets:join_room(topic, ws, payload)
    ...
    if not existing then
        ...
        local receiver = payload and payload.receiver or self:getSyncPrivateUserID(ws_id) -- Get the receiver user_id
        self:send_to_user(ws_id, {
...
function DawnSockets:setupWsChildProcess(ws_id, ws)
    local child = {
        ...
        start = function()
            self.connections[ws_id] = {
                ...
                state = {
                    ...
                    pending_acks = {} -- Store message IDs waiting for acknowledgement from this client
                }
            }
            self.shared.metrics.total_connections = (self.shared.metrics.total_connections or 0) + 1
            return true
        ...
        restart = function()
            ...
            -- self.supervisor:restartChild(child)
            return true
        end,
    }
...
function DawnSockets:handle_message(ws, message, opcode)
    print("[WS] Received message:", message, "opcode:", opcode)
    local ws_id = get_ws_id(ws)
    if not ws_id then
        error("Error: Unable to get WebSocket ID.")
        return
    ...
    elseif not conn.ws then
        ...
        elseif opcode == WS_OPCODE_BINARY then
            ...
            decoded.message_id = decoded.message_id or uuid.v4() -- Generate a unique message ID if not provided
            if event == "ack" and ack_id then
                ...
                if pending_callbacks and pending_callbacks[ack_id] then
                    ...
                    if callback then
                        pcall(callback, payload) -- Execute the acknowledgement callback
                    end
            ...
            if event == "join" and payload.sender then
                conn.state.user_id = payload.sender or conn.state.user_id -- Update user_id if provided
                conn.state.status = "online"
                self.state_management:set_user_status(payload.sender, conn.state.status) -- Set user status to online
                -- Check for queued messages
                local queuedMessages = self.state_management:fetch_queued_messages(conn.state.user_id)
                if queuedMessages and #queuedMessages > 0 then
                    for _, msg in ipairs(queuedMessages) do
                        msg.receiver = conn.state.user_id
                        msg.id = uuid.v4() -- Generate a unique ID for the message
                        self:send_to_user(ws_id, msg)
                    end
                    self.state_management:clear_queued_messages(conn.state.user_id)
                end
                self:syncPrivateChat(payload.sender, ws) -- Sync private chat
            end
...
function DawnSockets:send_to_user(ws_unique_identifier, message_table, ack_callback)
    ...
    local message_id = message_table.id -- Assuming your messages have a unique ID for tracking acknowledgements
    ...
    if ws and ws.send then
        ...
        if ack_callback and message_id then
            ...
            -- Optionally set a timeout for acknowledgements
            uv.timer_start(uv.new_timer(), 5000, 0, function() -- 5 seconds timeout
...
function DawnSockets:push_notification(ws, payload)
    ...
    local receiver = payload.receiver or self:getSyncPrivateUserID(ws_id) -- Get the receiver user_id
    return self:send_to_user(ws_id, {
        id = payload.id or uuid.v4(), -- Generate a unique ID for the notification
        receiver = receiver,
        type = "notification",
...
function DawnSockets:handle_close(ws, code, reason)
    local ws_id = get_ws_id(ws)
    if ws_id then
        local user_id = self:getSyncPrivateUserID(ws_id) or nil -- Get the user_id associated with the WebSocket ID
        if user_id then
            self.state_management:set_user_status(user_id, "offline") -- Set user status to offline
            -- self:syncPrivateChatLeave(user_id) -- Sync private chat leave with the user_id
        end
        -- Clean up any pending acknowledgements for this connection
...
-- Helper function to check if a room exists (assuming you add this to state_management)
function DawnSockets:room_exists(room_id)
    return self.state_management:room_exists(room_id)
end

-- Helper function to create a room (assuming you add this to state_management)
