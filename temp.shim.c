...
static uWS::WebSocket<false, true, DummyUserData>* check_websocket(lua_State *L, int index) {
    void *ud = luaL_checkudata(L, index, "websocket");
    if (!ud) luaL_error(L, "expected 'websocket' userdata");
    return *(uWS::WebSocket<false, true, DummyUserData>**)ud;
}

static int websocket_send(lua_State *L) {
    uWS::WebSocket<false, true, DummyUserData>* ws = check_websocket(L, 1);
    const char *message = luaL_checkstring(L, 2);
    size_t len = lua_objlen(L, 2); // Use lua_objlen for LuaJIT
    if (ws) {
        ws->send(std::string_view(message, len), uWS::OpCode::TEXT);
        lua_pushboolean(L, 1);
        return 1;
    } else {
...
static int websocket_send_binary(lua_State *L) {
    uWS::WebSocket<false, true, DummyUserData>* ws = check_websocket(L, 1);
    size_t len;
    const char *data = luaL_checklstring(L, 2, &len);
    if (ws) {
        ws->send(std::string_view(data, len), uWS::OpCode::BINARY);
        lua_pushboolean(L, 1);
        return 1;
    } else {
        luaL_error(L, "Invalid WebSocket object");
...
static int websocket_close(lua_State *L) {
    uWS::WebSocket<false, true, DummyUserData>* ws = check_websocket(L, 1);
    int code = luaL_optinteger(L, 2, 1000);
    const char *message = luaL_optstring(L, 3, "");
    if (ws) {
        ws->close(code, std::string_view(message));
        lua_pushboolean(L, 1);
        return 1;
    } else {
        luaL_error(L, "Invalid WebSocket object");
...
static int websocket_onData(lua_State *L) {
    uWS::WebSocket<false, true, DummyUserData>* ws = check_websocket(L, 1);
    ...
    if (ws) {
        ws->onMessage([L, ref](std::string_view message, uWS::OpCode opCode) {
            std::lock_guard<std::mutex> lock(lua_mutex);
            lua_rawgeti(L, LUA_REGISTRYINDEX, ref); // Get the Lua callback
            // Create a new Lua state for the callback to avoid issues
            lua_State *callback_L = lua_newthread(L);
            lua_xmove(L, callback_L, 1); // Move the callback to the new thread

            lua_pushlightuserdata(callback_L, (void*)message.data());
            lua_pushinteger(callback_L, message.size());
            lua_pushinteger(callback_L, static_cast<int>(opCode));

            if (lua_pcall(callback_L, 3, 0, 0) != LUA_OK) {
                std::cerr << "Lua error in onData callback: " << lua_tostring(callback_L, -1) << std::endl;
            }
            // We don't pop the callback from the original L as it's still referenced
        });
        lua_pushboolean(L, 1);
        return 1;
    } else {
...
static void create_websocket_metatable(lua_State *L) {
    ...
    lua_pushcfunction(L, websocket_send_binary);
    lua_setfield(L, -2, "sendBinary");
    lua_pushcfunction(L, websocket_close);
    lua_setfield(L, -2, "close");
    lua_pushcfunction(L, websocket_onData);
    lua_setfield(L, -2, "onData");
    ...
    app->ws<DummyUserData>(route, {
        ...
        .message = [callback_id](auto *ws, std::string_view message, uWS::OpCode opCode) {
            ...
            *ws_ud = ws;
            ...
            lua_pushinteger(main_L, static_cast<int>(opCode)); // Pass the OpCode
            lua_pushlightuserdata(main_L, (void*)message.data()); // Pass data pointer
            lua_pushinteger(main_L, message.size());           // Pass data size
        ...
        .drain = [callback_id](auto *ws) {
            ...
            *ws_ud = ws;
            ...
            lua_pushstring(main_L, "drain");

            if (lua_pcall(main_L, 2, 0, 0) != LUA_OK) {
                std::cerr << "Lua error (drain): " << lua_tostring(main_L, -1) << std::endl;
                lua_pop(main_L, 1);
            }
        },

        .ping = [callback_id](auto *ws) {
            std::lock_guard<std::mutex> lock(lua_mutex);
            lua_rawgeti(main_L, LUA_REGISTRYINDEX, lua_callbacks[callback_id]);
            ...
            *ws_ud = ws;
            ...
            lua_pushstring(main_L, "ping");

            if (lua_pcall(main_L, 2, 0, 0) != LUA_OK) {
                std::cerr << "Lua error (ping): " << lua_tostring(main_L, -1) << std::endl;
                lua_pop(main_L, 1);
            }
        },

        .pong = [callback_id](auto *ws) {
            std::lock_guard<std::mutex> lock(lua_mutex);
            lua_rawgeti(main_L, LUA_REGISTRYINDEX, lua_callbacks[callback_id]);
            ...
            *ws_ud = ws;
            ...
            lua_pushstring(main_L, "pong");

            if (lua_pcall(main_L, 2, 0, 0) != LUA_OK) {
                std::cerr << "Lua error (pong): " << lua_tostring(main_L, -1) << std::endl;
                lua_pop(main_L, 1);
            }
        },
...
static void create_websocket_metatable(lua_State *L) {
    ...
    lua_pushcfunction(L, websocket_send);
    lua_setfield(L, -2, "send");
    lua_pushcfunction(L, websocket_send_binary);lua_setfield(L, -2, "sendBinary");
    lua_pushcfunction(L, websocket_close);
    lua_setfield(L, -2, "close");
    lua_pushcfunction(L, websocket_onData);
    lua_setfield(L, -2, "onData");
